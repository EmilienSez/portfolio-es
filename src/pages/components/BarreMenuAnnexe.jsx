...
export default function BarreMenuAnnexe({ nom, couleur, link, type_menu, activeItem, onSousMenuClick }) {
  ...
  return (
    <div>
      <Link to={link}>
      <p
        className={`font-oswald-bold text-center text-3xl mt-2 tracking-wider ${nom == type_menu ? "bg-[#ff3131]" : ""} text-[#1a1a1a] dark:text-white ${nom == type_menu ? "dark:bg-[#ff3131]" : ""} dark:hover:bg-[${couleur}] hover:bg-[${couleur}] transition-colors duration-300`}
      >{nom}    </p>
      </Link>
